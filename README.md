...
**静默模式（默认）**：所有 `goto` / 点击 / 填表都在专用 agent 页签后台执行，**不会抢走你正在用的页签**。  
...
2. 打开右上角 **开发者模式**
3. **加载已解压的扩展程序**
4. 选择仓库里的 `extension/` 目录（根目录没有 `manifest.json`，选根目录会加载失败）
5. 点插件图标会直接打开侧边栏

侧边栏若还没连上服务，会显示安装命令。服务起来后会自动连上。
...
- 服务**只监听** `127.0.0.1`，外网访问不到。
- 本机 token 只存在 `.bridge-state.json`（已 gitignore）和本机 Chrome 扩展存储里，**不会上传**。
- token 不是账号密码，也不是 xAI / Google 的云端密钥。本机上拿到它的程序可以向桥发送命令，所以不要复制到聊天、Issue 或截图里。
- 插件只在你自己的 Chrome 里跑；截图、点选、页面内容都留在本机。
- 不要把这个桥当成无人值守的站点运营工具。发帖、付款、删数据这类操作应先确认。
